import React from "react";
import cartproductdata from "./cartProductData";

const CartProduct = () => {
  return (
    <div>
      <div className="hidden lg:block">
        <div className="flex mb-12">
          <div className="w-[290px] ">
            <h3 className="text-blue text-[20px] font-bold font-lato">
              Product
            </h3>
          </div>

          <div className="w-[170px] ml-6">
            <h3 className="text-blue text-[20px] font-bold font-lato">
              {" "}
              Price
            </h3>
          </div>

          <div className="w-[150px] ">
            <h3 className="text-blue text-[20px] font-bold font-lato">
              Quantity
            </h3>
          </div>

          <div className="w-[60px] ml-9">
            <h3 className="text-blue text-[20px] font-bold font-lato">Total</h3>
          </div>
        </div>
      </div>

      <div className="">
        {cartproductdata.map((item) => {
          return (
            <div>
              <div className="flex items-center  md:mb-8 mb-4">
                <div className="w-[60px] md:w-[90px] ">
                  <img src={item.pic} alt="shopping items" />
                </div>

                <div className=" w-[120px]  md:w-[170px] ml-1 md:ml-4 ">
                  <h5 className="text-[10px] md:text-[14px] font-josefin font-bold">
                    {item.title}
                  </h5>
                  <p className="text-[10px] md:text-[12px] text-[#9295AA]">
                    Color:{" "}
                    <span className="text-[12px] md:text-[14px]">
                      {item.color}
                    </span>
                  </p>
                  <p className="text-[10px] md:text-[12px] text-[#9295AA]">
                    Size:{" "}
                    <span className="text-[12px] md:text-[14px]">
                      {item.size}
                    </span>
                  </p>
                </div>

                <div className="md:ml-10 md:w-[55px] ">
                  <h5 className="text-[10px] md:text-[14px] font-bold text-blue font-josefin">
                    {item.price}
                  </h5>
                </div>

                <div className="md:ml-28  ml-4 md:w-[60px] flex items-center justify-between bg-[#F0EFF2]">
                  <div className="bg-[#E7E7EF] px-[5px] text-[#BEBFC2]">
                    <h5>-</h5>
                  </div>
                  <h5 className="text-[12px] text-[#BEBFC2] font-josefin">
                    {item.quantity}
                  </h5>
                  <div className="bg-[#E7E7EF] px-1 text-[#BEBFC2]">
                    <h5>+</h5>
                  </div>
                </div>

                <div className="md:ml-32 ml-4 md:w-[60px] ">
                  <h5 className="text-blue font-bold font-josefin ">
                    {item.totalPrice}
                  </h5>
                </div>
              </div>

              <div className="border-[1px] -mt-2 md:mb-6 mb-2"></div>
            </div>
          );
        })}
      </div>

      <div className="flex justify-between mt-2 md:mt-8 mb-12">
        <button className="bg-pink text-white text-[14px] md:text-[17px] md:px-11 px-8 md:py-3 py-2 rounded-sm font-josefin font-medium hover:bg-[#d1236e] ">
          Update cart
        </button>
        <button className="bg-pink text-white text-[14px] md:text-[17px] md:px-11 px-8 md:py-3 py-2 rounded-sm font-josefin font-medium hover:bg-[#d1236e] ">
          Clear cart
        </button>
      </div>
    </div>
  );
};

export default CartProduct;
